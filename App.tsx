...
import { Word, HSKLevel, AppMode, Difficulty, User } from './types';
...
import { Layers, Book, GraduationCap, Menu, List, X as XIcon, LogOut, User as UserIcon, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    if (user) {
      const userProgress = authService.loadProgress(user.username);
  ...
  const handleToggleStatus = (wordId: string, isMastered: boolean) => {
    ...
    authService.saveProgress(user.username, newSet);
    ...
    <div className="flex h-full flex-col md:flex-row bg-slate-50">
      ...
      `}>
        <div className={`
          ...
          `}>
             <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 flex-shrink-0" title={user.username}>
          ...
          <div className="mt-auto p-4 border-t border-slate-100 bg-slate-50 space-y-4">
             ...
             <button 
               ...
               title="Logout"
               className={`
                 w-full py-2 rounded-lg border border-slate-200 text-slate-500 text-sm font-medium hover:bg-red-50 hover:text-red-600 hover:border-red-100 transition-colors flex items-center gap-2
                 ${isCollapsed ? 'justify-center px-2' : 'px-4 justify-center'}
               `}
             >
